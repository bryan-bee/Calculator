...
public class DList<E> {
    ...
    /**
     * For a nonempty DList, mHead is a reference to the first Node in the
     * DList. For an empty DList, mHead will be null.
     */
    private Node<E> mHead;

    /**
     * The number of Nodes containing data in this DList. For an empty DList
     * mSize is 0.
     */
    private int mSize;

    /**
     * For a nonempty DList mTail is a reference to the last Node in the DList.
     * For an empty DList, mTail will be null.
     */
    private Node<E> mTail;

    /**
     * Creates an empty DList. For an empty DList, mHead = null, mTail = null,
     * and mSize = 0.
     */
    public DList() {
        setHead(null);
    ...
    /**
     * Inserts a new Node containing pData into this DList at index pIndex.
     * Shifts the element currently at that index (if it exists) and succeeding
     * elements to the right (i.e., adds one to their indices).
     *
     * Note that if pIndex == getSize() then new element is appended to the
     * list.
     *
     * Throws an IndexOutOfBoundsException if pIndex < 0 or pIndex > size of the
     * DList.
     */
    ...
    /**
     * Appends a new Node storing pData to this DList. Note that passing an
     * index of getSize() to add(int pIndex, E pData) will cause add() to
    ...
    /**
     * Removes all of the elements from the DList. After this operation the
     * DList will be empty.
     */
    public void clear() {
    ...
    /**
     * Removes the element at index pIndex from this DList. Shifts any
     * succeeding elements to the left (i.e., subtracts one from their indices).
     ...
     * specified element. Returns the element that was previously stored at
     * pIndex.
     */
    ...
    /**
     * Returns a string representation of this DList where we define string
     * representation be the string representation of each of the Nodes.
     */
    @Override
    ...
    protected static class Node<E> {
        ...
        /**
         * Creates a new Node storing no data and with mNext and mPrev set to
         * null.
         */
        public Node() {
            this(null);
        ...
        /**
         * Creates a new Node storing pData as the data and with mNext and mPrev
         * set to null.
         */
        public Node(E pData) {
            setData(pData);
        ...
        /**
         * Creates a new Node storing pData as the data, mPrev initialized to
         * pPrev, and mNext initialized to pNext.
         */
        ...
        /**
         * Returns true if this Node and pNode are equal to each other where
         * equal is defined as:
         *
         * 1. this Node cannot be null because if it were we would not have been
         * able to call the equals() method on it. However, pNode might be null
         * and if it is then clearly this Node and pNode are not equal so we
         * return false. 2. If the address of this Node and pNode are the same
         * then they are the same object so they are clearly equal to each other
         * so we return true. 3. If this Node and pNode are objects of different
         * classes then they cannot be equal so we return false. 4. If the
         * instance variables of this Node are equal to the instance variables
         * of pNode then the two objects are considered equal so return true. 5.
         * Otherwise, returns false.
         *
         * Also, the parameter object to equals() has to be of type Object
         * because we are over- riding the equals() method declared in Object
         * and for Object.equals() the paramemter is an Object. However, we need
         * to call the getData(), getNext(), and getPrev() methods on pNode and
         * we cannot do that if the JVM thinks that pNode is an Object.
         * Consequently we must type case pNode to be a Node<E>. Unfortunately,
         * if you compile with the -Xlint option you will get an "unchecked
         * cast" warning on the line where we declare node and type cast pNode
         * to Node<E>. For reasons I have not taken the time to fully understand
         * it is difficult to make this warning go away so a quick and easy and
         * sleazy way to eliminate the warning is to provide a
         * @SuppressWarnings("unchecked") attribute which tells the compiler not
         * to complain about this uncheck cast.
         *
        ...
        /**
         * Returns a string representation of this Node where we define the
         * string representation to be the string representation of the data
         * stored in this Node.
         */
        @Override
        public String toString() {
